...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		colorCorrectionShader->LoadShaderPartFromFile("shaders/Post/ColorCorrection.glsl", GL_FRAGMENT_SHADER);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effects"))
			{
				...
				if (activeEffect == 2)
				{
					...
					LUT3D lut = temp->GetLUT();
					temp->SetLUT(lut);
					/*if (ImGui::Button("ColorGrade", &lut))
					{
						temp->SetLUT(lut);
					}*/
				}
			...
			if (ImGui::CollapsingHeader("Light/Texture Settings"))
			{
				...
				if (ImGui::Button("Ambient + Specular + Bloom"))
				{
					...
					//shader->SetUniform("uPixelSize", 1.0f);
					shader->SetUniform("uThreshold", 1.0f);
				}
		...
		Texture2D::sptr Ground = Texture2D::LoadFromFile("images/NewGround.png");
		...
		Texture2D::sptr berch = Texture2D::LoadFromFile("images/Berch.png");
		Texture2D::sptr barrel = Texture2D::LoadFromFile("images/BarrelTexture.png");
		Texture2D::sptr cannapi = Texture2D::LoadFromFile("images/CannapiTexture.png");
		Texture2D::sptr Blank = Texture2D::LoadFromFile("images/NoTexture.png");

		LUT3D testCube("cubes/ShiftedBrightenedCorrection.cube");
		...
		GroundMat->Set("s_Specular", noSpec);
		GroundMat->Set("u_Shininess", 2.0f);
		GroundMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr BerchMat = ShaderMaterial::Create();
		BerchMat->Shader = shader;
		BerchMat->Set("s_Diffuse", berch);
		BerchMat->Set("s_Specular", noSpec);
		BerchMat->Set("u_Shininess", 2.0f);
		BerchMat->Set("u_TextureMix", 0.0f);
		...
		ShaderMaterial::sptr CannapiMat = ShaderMaterial::Create();
		CannapiMat->Shader = shader;
		CannapiMat->Set("s_Diffuse", cannapi);
		CannapiMat->Set("s_Specular", noSpec);
		CannapiMat->Set("u_Shininess", 2.0f);
		CannapiMat->Set("u_TextureMix", 0.0f);
		...
		GameObject obj1 = scene->CreateEntity("Ground"); 
		{
			...
			obj1.emplace<RendererComponent>().SetMesh(vao).SetMaterial(GroundMat);
		...
		GameObject BerchTree = scene->CreateEntity("BerchTree");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/DeadBerchTree.obj");
			BerchTree.emplace<RendererComponent>().SetMesh(vao).SetMaterial(BerchMat);
			BerchTree.get<Transform>().SetLocalPosition(-5.0f, 5.0f, -0.3f);
			BerchTree.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(BerchTree);

		}

		GameObject BerchTree2 = scene->CreateEntity("BerchTree2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/DeadBerchTree.obj");
			BerchTree2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(BerchMat);
			BerchTree2.get<Transform>().SetLocalPosition(5.0f, 5.0f, -0.3f);
			BerchTree2.get<Transform>().SetLocalRotation(90.0f, 0.0f, -30.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(BerchTree2);
		}

		GameObject Barrel = scene->CreateEntity("Barrel");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Barrel.obj");
			Barrel.emplace<RendererComponent>().SetMesh(vao).SetMaterial(BarrelMat);
			Barrel.get<Transform>().SetLocalPosition(1.0f, -0.5f, 0.0f);
			Barrel.get<Transform>().SetLocalRotation(90.0f, 0.0f, -30.0f);
			Barrel.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(Barrel);
		}

		GameObject Barrel2 = scene->CreateEntity("Barrel2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Barrel.obj");
			Barrel2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(BarrelMat);
			Barrel2.get<Transform>().SetLocalPosition(-4.0f, -3.0f, 0.3f);
			Barrel2.get<Transform>().SetLocalRotation(0.0f, 0.0f, 0.0f);
			Barrel2.get<Transform>().SetLocalScale(0.3f, 0.3f, 0.3f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(Barrel2);
		}

		GameObject Cannapi = scene->CreateEntity("Cannapi");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Cannapi.obj");
			Cannapi.emplace<RendererComponent>().SetMesh(vao).SetMaterial(CannapiMat);
			Cannapi.get<Transform>().SetLocalPosition(0.0f, 1.5f, 0.0f);
			Cannapi.get<Transform>().SetLocalRotation(90.0f, 0.0f, 90.0f);
			Cannapi.get<Transform>().SetLocalScale(0.5f, 0.5f, 0.5f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(Cannapi);
		}
		...
		GameObject colorcorrectObject = scene->CreateEntity("ColorCorrect Effect");
		{
			colorcorrectEffect = &colorcorrectObject.emplace<ColorCorrection>();
		...
		{
			bloomEffect = &colorcorrectObject.emplace<BloomEffect>();
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			{
				GroundMat->Set("s_Diffuse", Blank);
				BerchMat->Set("s_Diffuse", Blank);
				BarrelMat->Set("s_Diffuse", Blank);
				CannapiMat->Set("s_Diffuse", Blank);
			}

			glm::vec3 startPos = glm::vec3(-4.0f, -3.0f, 0.3f);
			glm::vec3 endPos = glm::vec3(4.0f, -3.0f, 0.3f);
				
			
			/*if (Barrel2.get<Transform>().GetLocalPosition() == startPos)
			{
				Barrel2.get<Transform>().SetLocalPosition(LERP(startPos, endPos, 0.2));
				std::cout << "is Moving left" << std::endl;
			}
				
			else if (Barrel2.get<Transform>().GetLocalPosition() == endPos)
			{
				Barrel2.get<Transform>().SetLocalPosition(LERP(endPos, startPos, 0.2f));
			...
			if (BarrelMove == true)
			{
				Barrel2.get<Transform>().SetLocalPosition(LERP(startPos, endPos, t));
			}
			else if (BarrelMove == false)
			{
				Barrel2.get<Transform>().SetLocalPosition(LERP(endPos, startPos, t));
			}


			if (Barrel.get<Transform>().GetLocalPosition() == startPos)
			{
				BarrelMove = true;
			}

			if (Barrel.get<Transform>().GetLocalPosition() == endPos)
			{
				BarrelMove = false;
